...
class JG_Map_Enqueue {
    ...
    public function render_top_bar() {
        ...
        (function () {
            ...
            var DURATION    = 300; /* ms – matches CSS right-transition */
            ...
            /* Find the two direct child columns sharing a common flex parent.
               Uses the common-ancestor approach so we never target a shared wrapper. */
            function getLayoutCols() {
                var sidebar = document.getElementById('jg-map-sidebar');
                var mapWrap = document.getElementById('jg-map-wrap');
                if (!sidebar || !mapWrap) return null;

                function ancestors(el) {
                    var list = [], node = el.parentElement;
                    while (node) { list.push(node); node = node.parentElement; }
                    return list;
                }

                var sa = ancestors(sidebar);
                var ma = ancestors(mapWrap);
                for (var i = 0; i < sa.length; i++) {
                    var j = ma.indexOf(sa[i]);
                    if (j !== -1 && i > 0 && j > 0) {
                        return { sidebarCol: sa[i - 1], mapCol: ma[j - 1] };
                    }
                }
                return null;
            }

            /* Position the tab without the right-transition (used on load/resize) */
            function positionTabInstant() {
                if (window.innerWidth <= 768) return;
                /* Temporarily suspend right-transition */
                tab.style.transition = 'background 0.2s, opacity 0.2s';
                if (sidebarHidden) {
                    tab.style.right = '0px';
                } else {
                    var sidebar = document.getElementById('jg-map-sidebar');
                ...
                /* Restore full transition after one frame */
            ...
            function collapse() {
                ...
                var cols = getLayoutCols();
                if (!cols) { isAnimating = false; return; }
                var sCol = cols.sidebarCol;
                var mCol = cols.mapCol;

                /* Capture current pixel widths */
                var sW    = sCol.getBoundingClientRect().width;
                var mW    = mCol.getBoundingClientRect().width;
                var total = sW + mW;

                /* Update button icon + animate right immediately */
                tab.innerHTML = SVG_RIGHT;
                tab.title     = 'Pokaż listę miejsc';
                tab.style.right = '0px'; /* CSS transition moves it */

                document.body.classList.add('jg-sidebar-hidden');

                /* Pin columns to pixel widths before animating */
                sCol.style.setProperty('overflow',  'hidden',             'important');
                sCol.style.setProperty('flex',      '0 0 ' + sW + 'px',  'important');
                sCol.style.setProperty('width',     sW + 'px',            'important');
                sCol.style.setProperty('max-width', sW + 'px',            'important');
                mCol.style.setProperty('flex',      '0 0 ' + mW + 'px',  'important');
                mCol.style.setProperty('width',     mW + 'px',            'important');
                mCol.style.setProperty('max-width', mW + 'px',            'important');
                void sCol.offsetWidth; /* force reflow */

                var tr = 'width ' + DURATION + 'ms ease, max-width ' + DURATION + 'ms ease, flex-basis ' + DURATION + 'ms ease';
                sCol.style.setProperty('transition', tr, 'important');
                mCol.style.setProperty('transition', tr, 'important');

                /* Animate */
                sCol.style.setProperty('flex',      '0 0 0px',          'important');
                sCol.style.setProperty('width',     '0px',              'important');
                sCol.style.setProperty('max-width', '0px',              'important');
                mCol.style.setProperty('flex',      '0 0 ' + total + 'px', 'important');
                mCol.style.setProperty('width',     total + 'px',       'important');
                mCol.style.setProperty('max-width', total + 'px',       'important');

                setTimeout(function () {
                    sCol.style.setProperty('display', 'none', 'important');
                    sCol.style.removeProperty('transition');
                    sCol.style.removeProperty('overflow');
                    mCol.style.setProperty('flex',      '0 0 100%', 'important');
                    mCol.style.setProperty('width',     '100%',     'important');
                    mCol.style.setProperty('max-width', '100%',     'important');
                    mCol.style.removeProperty('transition');
                    window.dispatchEvent(new Event('resize'));
                    isAnimating = false;
                }, DURATION + 20);
            ...
            function expand() {
                ...
                var total   = mCol.getBoundingClientRect().width; /* 100% currently */
                var targetS = total * 0.20;
                var targetM = total * 0.80;

                document.body.classList.remove('jg-sidebar-hidden');

                /* Show sidebar at 0 width so it doesn't flash */
                sCol.style.removeProperty('display');
                sCol.style.setProperty('overflow',  'hidden',        'important');
                sCol.style.setProperty('flex',      '0 0 0px',       'important');
                sCol.style.setProperty('width',     '0px',           'important');
                sCol.style.setProperty('max-width', '0px',           'important');
                ...
                /* Update button BEFORE animation so it moves in sync */
                tab.innerHTML   = SVG_LEFT;
                tab.title       = 'Ukryj listę miejsc';
                tab.style.right = targetS + 'px'; /* CSS transition moves it */
                ...
                /* Animate */
                sCol.style.setProperty('flex',      '0 0 ' + targetS + 'px', 'important');
                sCol.style.setProperty('width',     targetS + 'px',          'important');
                sCol.style.setProperty('max-width', targetS + 'px',          'important');
                mCol.style.setProperty('flex',      '0 0 ' + targetM + 'px', 'important');
                mCol.style.setProperty('width',     targetM + 'px',          'important');
                mCol.style.setProperty('max-width', targetM + 'px',          'important');

                setTimeout(function () {
                    sCol.style.removeProperty('transition');
                    sCol.style.removeProperty('overflow');
                    sCol.style.setProperty('flex',      '0 0 20%', 'important');
                    sCol.style.setProperty('width',     '20%',     'important');
                    sCol.style.setProperty('max-width', '20%',     'important');
                    mCol.style.removeProperty('transition');
                    mCol.style.setProperty('flex',      '0 0 80%', 'important');
                    mCol.style.setProperty('width',     '80%',     'important');
                    mCol.style.setProperty('max-width', '80%',     'important');
                    positionTabInstant(); /* final exact position */
                    window.dispatchEvent(new Event('resize'));
                    isAnimating = false;
                }, DURATION + 20);
            ...
            /* Initial hidden-state applied instantly (no animation on page load) */
            function applyInitialHidden(cols) {
                var sCol = cols.sidebarCol;
                var mCol = cols.mapCol;
                sCol.style.setProperty('display',   'none', 'important');
                mCol.style.setProperty('flex',      '0 0 100%', 'important');
            ...
            function init() {
                ...
                if (saved) {
                    ...
                    tab.style.opacity = '1';
                    var cols = getLayoutCols();
                    if (cols) applyInitialHidden(cols);
                    /* Retry in case Elementor hasn't rendered yet */
                    function retryHidden(n) {
                        var c = getLayoutCols();
                        if (c) { applyInitialHidden(c); }
                        else if (n > 0) { requestAnimationFrame(function() { retryHidden(n - 1); }); }
                    }
                    requestAnimationFrame(function() { retryHidden(60); });
                } else {
                    /* Wait for sidebar to be rendered/measured before showing tab */
                    var attempts = 0;
                    function tryPosition() {
                        ...
                        if (sidebar) {
                            ...
                            if (rect.width > 0) {
                                ...
                                tab.style.right   = Math.max(0, window.innerWidth - rect.left) + 'px';
                                tab.style.opacity = '1';
                                requestAnimationFrame(function() { tab.style.removeProperty('transition'); });
                                return;
                            }
                        }
                        if (attempts < 240) { requestAnimationFrame(tryPosition); }
                        else { tab.style.right = '0px'; tab.style.opacity = '1'; }
                    }
                    requestAnimationFrame(tryPosition);
                }

                window.addEventListener('resize', function () { if (!isAnimating) positionTabInstant(); });
                window.addEventListener('load',   function () { if (!isAnimating) positionTabInstant(); });
            }

            tab.addEventListener('click', function () {
                if (isAnimating) return;
                sidebarHidden ? expand() : collapse();
            });
