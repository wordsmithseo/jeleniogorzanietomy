...
class JG_Interactive_Map {
    ...
    private function init_hooks() {
        ...
        // Search engine ping cron handler
        add_action('jg_map_ping_search_engines', array(__CLASS__, 'execute_search_engine_ping'));

        // Create ping log table on plugins_loaded (ensures it exists)
        add_action('plugins_loaded', array(__CLASS__, 'ensure_ping_log_table'));
...
<?php endforeach; ?>
     ...
     * Schedules a background cron event for retry with logging.
     *
     * @param int $point_id The ID of the published point
     */
    public static function ping_search_engines($point_id) {
        // Schedule the ping as a one-time cron event so it runs in the background
        // with retry capability and doesn't slow down the request.
    ...
    /**
     * Log a search engine ping result to the database.
     */
    private static function log_ping($engine, $point_id, $point_url, $success, $http_code, $attempts, $error_message) {
        global $wpdb;
        $table = $wpdb->prefix . 'jg_map_ping_log';
    ...
    /**
     * Create the ping log database table.
     */
    public static function create_ping_log_table() {
        global $wpdb;
        $table = $wpdb->prefix . 'jg_map_ping_log';
    ...
    /**
     * Ensure ping log table exists (called on plugins_loaded).
     */
    public static function ensure_ping_log_table() {
        global $wpdb;
        $table = $wpdb->prefix . 'jg_map_ping_log';
        $table_exists = $wpdb->get_var("SHOW TABLES LIKE '$table'");
        if ($table_exists !== $table) {
            self::create_ping_log_table();
        }
    }

    /**
     * Get ping log entries for admin display.
     *
     * @param int $limit Number of entries to return
     * @return array
     */
    public static function get_ping_log($limit = 100) {
        global $wpdb;
        $table = $wpdb->prefix . 'jg_map_ping_log';
