    ...
    try {
      ...
      function openDetailsModalContent(p) {
        ...
        if (p.phone || p.email || p.website) {
          ...
          if (p.email) {
            kontaktItems.push('<div><button type="button" class="jg-place-contact-open-btn" data-point-id="' + esc(String(p.id)) + '"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg> Napisz wiadomość</button></div>');
          }
          if (p.website) {
